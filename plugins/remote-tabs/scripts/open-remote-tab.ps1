...
if ($py -and (Test-Path $hostPy)) {
    ...
    # (pty_host's local-input forwarding is left disabled — console = None in
    # pty_host.py main() — preserved for a future "minimized window + local edit"
    # option.)
    $pyExe = $py[0]; $pyPre = $py[1]
    # Launch pty_host so NO console window ever flashes AND the launcher never blocks.
